    ...
    <div>
      <table class="Table">
        ...
        <tr class="Row" v-for="(product,index) in products" :key="index" v-bind:style="styleobj">
          ...
          <td>{{product.vat}}</td>
          <td>
            <button v-on:click="deleteproduct(product.id)">remove item</button>
          </td>
        </tr>
      </table>
...
export default {
  ...
  components: {
    productcomponent: {
      template:
        '<div class="Table"><div class="Row" v-bind:style="styleobj"><div class="Cell"><p>{{product.name}}</p></div><div class="Cell"><p>{{product.price}}</p></div></div></div>',
      props: ["product", "index"],
      data: function() {
        return {
          styleobj: {
